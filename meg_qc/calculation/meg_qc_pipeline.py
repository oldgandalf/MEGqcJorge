...
def create_summary_report(
    ...
    json_file : str or Path
        Path to the intermediate metrics JSON generated by the pipeline.
    html_output : str | None
        Optional HTML output path. If ``None`` only ``json_output`` is written.
    json_output : str
        Path of the JSON summary that will be created.
    gqi_settings : dict | None
        Dictionary with thresholds and weights for the Global Quality Index.
        It may also contain ``compute_gqi`` and ``include_ecg_eog`` booleans
        to control the index computation.
    """
    # === Load JSON ===
    ...
    def build_summary_table(source):
        """Create a summary DataFrame listing noisy and flat channels."""
        rows = []
    ...
    def build_psd_summary(noise_mag, noise_grad):
        """Return a DataFrame summarising the percentage of noisy power."""
        df = pd.DataFrame([
            {
                "Metric": "Noise Power",
                "mag": noise_mag,
    ...
    if gqi_settings is not None:
        thresholds = {
            k: v for k, v in gqi_settings.items() if isinstance(v, dict)
        }
    else:
        thresholds = {
    ...
    def quality_q(M, start, end):
        """Map a metric value ``M`` onto a 0-1 quality scale."""
        if M <= start:
            return 1.0
        elif M >= end:
            return 0.0
    ...
    def count_high_correlations_from_details(section, contamination_key):
        """Count channels with strong ECG/EOG correlation."""
        results = []
        percentages = []
        for sensor_type in ["mag", "grad"]:
            entries = (
    ...
    if weight_ecg:
        q_corr_ecg = 0.5 if ecg_noisy else quality_q(
            ecg_pct, thresholds["corr"]["start"], thresholds["corr"]["end"]
        )
    else:
        q_corr_ecg = 1.0

    if weight_eog:
        q_corr_eog = 0.5 if eog_noisy else quality_q(
            eog_pct, thresholds["corr"]["start"], thresholds["corr"]["end"]
        )
    else:
        q_corr_eog = 1.0
    ...
    weights_sum = (
        thresholds["ch"]["weight"]
        + weight_ecg
        + weight_eog
        + thresholds["mus"]["weight"]
        + thresholds["psd"]["weight"]
    )
    if weights_sum == 0:
        weights_sum = 1.0
    ...
    if compute_gqi:
        ...
        penalties = {
            ...
            * (1 - q_ch)
            ...
            "corr": 100
            * (
                weight_ecg * (1 - q_corr_ecg)
                + weight_eog * (1 - q_corr_eog)
            )
            / weights_sum,
            "mus": 100
            * thresholds["mus"]["weight"]
        ...
        try:
            ...
            if rows:
                group_dir = os.path.join(
                    dataset_path, "derivatives", "Meg_QC", "Group_metrics"
                )
                os.makedirs(group_dir, exist_ok=True)
                df = pd.DataFrame(rows)
                df.to_csv(
                    os.path.join(group_dir, "Global_Quality_Index.tsv"),
                    sep="\t",
                    index=False,
                )
        except Exception as e:
            print("___MEGqc___: Failed to create group metrics", e)
